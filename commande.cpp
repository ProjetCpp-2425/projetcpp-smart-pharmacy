...
{
    QSqlQuery query;
    query.prepare("DELETE FROM COMMANDE WHERE code_commande = :codeasupprimer");
...
Commande Commande::trouver(int code)
{
    ...
    query.prepare("SELECT code_commande, adresse, date_commande, status_commande, remarque, codef, cin, produit "
                  "FROM COMMANDE WHERE code_commande = :code");
...
QSqlQueryModel* Commande::rechercher(QString critere) {
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query;

    query.prepare(
           "SELECT code_commande, adresse, date_commande, status_commande, remarque, codef, cin, produit "
           "FROM COMMANDE "
           "WHERE code_commande LIKE :critere OR "
           "adresse LIKE :critere OR "
           "date_commande LIKE :critere OR "
           "status_commande LIKE :critere OR "
           "remarque LIKE :critere OR "
           "codef LIKE :critere OR "
           "cin LIKE :critere OR "
           "produit LIKE :critere"
       );
